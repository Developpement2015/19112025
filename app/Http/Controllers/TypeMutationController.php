...
class TypeMutationController extends Controller
{
    public function page()
    {
        return view('parameters.type_mutations.page');
    }
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Type de mutation ajouté avec succès');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Une erreur s\'est produite lors de l\'ajout de la type de mutation.');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $service = TypeMutation::find($id);
            $service->nom = $request->input('nom') ?? $service->nom;
            $service->nom_arabe = $request->input('nom_arabe') ?? $service->nom_arabe;
            $service->save();

            return redirect()->back()->with('success', 'Type de mutation mise à jour avec succès');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Une erreur s\'est produite lors de la mise à jour de la Type de mutation.');
        }
    }
    ...
    {
        $query = TypeMutation::query();
        $query->where('id', '!=', null);
        return DataTables::of($query)
            ->addColumn('action', function ($data) {
                return view('parameters.type_mutations.action', compact('data'))->render();
            })
            ->rawColumns(['action'])
            ->toJson();
    }
    ...
    {
        TypeMutation::where('id', $id)->delete();
        return redirect()->back()->with('success', 'Type de mutation supprimé avec succès');
    }

    public function export()
    {
        $data = TypeMutation::all(['nom', 'nom_arabe']);
        ...
        $fileName = 'Type_de_mutation.xlsx';
    ...
    public function pdf()
    {
        ...
        // Add content
        $html = '<h2 style="text-align: center;">Les types de mutation</h2>';
