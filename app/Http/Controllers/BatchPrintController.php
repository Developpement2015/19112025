...
class BatchPrintController extends Controller
{
    ...
    public function exportCongeDecisions(Request $request)
    {
        ...
        foreach ($decisions as $index => $decision) {
            $row = $index + 2;
            $sheet->setCellValue('A' . $row, optional($decision->fonctionnaire)->ppr ?? '');
            $sheet->setCellValue('B' . $row, optional($decision->fonctionnaire)->nom ?? '');
            $sheet->setCellValue('C' . $row, optional($decision->fonctionnaire)->prenom ?? '');
            $sheet->setCellValue('D' . $row, optional(optional($decision->fonctionnaire)->nomFormationSanitaire)->nom ?? 'N/A');
            $sheet->setCellValue('E' . $row, optional(optional($decision->fonctionnaire)->nomFormationSanitaire)->nom ?? 'N/A');
            $sheet->setCellValue('F' . $row, optional(optional($decision->fonctionnaire)->service)->nom ?? 'N/A');
            $sheet->setCellValue('G' . $row, optional($decision->typeConge)->nom ?? 'N/A');
