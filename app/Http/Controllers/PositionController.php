...
class PositionController  extends Controller
{
    ...
    {
        return view('parameters.positions.page');
    }
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Position ajouté avec succès');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Une erreur s\'est produite lors de l\'ajout de la Position.');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $position->nom = $request->input('nom') ?? $position->nom;
            $position->nom_arabe = $request->input('nom_arabe') ?? $position->nom_arabe;
            $position->save();

            return redirect()->back()->with('success', 'Position mise à jour avec succès');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Une erreur s\'est produite lors de la mise à jour de la position.');
        }
    }
    ...
    public function data(Request $request)
    {
        ...
        return DataTables::of($query)
            ->addColumn('action', function ($data) {
                return view('parameters.positions.action', compact('data'))->render();
            })
            ->rawColumns(['action'])
            ->toJson();
    }
    ...
    public function delete($id)
    {
        ...
        return redirect()->back()->with('success', 'Position supprimé avec succès');
    }

    public function export()
    {
        $data = Position::all(['nom', 'nom_arabe']);
    ...
    public function pdf()
    {
        ...
        // Add content
        $html = '<h2 style="text-align: center;">Les Positions</h2>';
