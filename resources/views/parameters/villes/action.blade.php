            ...
            <div class="page-wrapper-new p-0">
                <div class="content">
                    ...
                    <div class="modal-body custom-modal-body">
                        <form action="{{ route('villes.update', $data->id) }}" method="post">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" name="region_id">
                                            @foreach (\App\Models\Region::all() as $region)
                                                <option @selected($region->id == $data->region_id) value="{{ $region->id }}">{{ $region->nom }} -
...
<!-- modifier - modal-->
    <a class="me-0 btn btn-danger  p-2 mb-0" href="javascript:void(0);"
        onclick="confirmDelete({{ $data->id }}, '{{ route('villes.delete', $data->id) }}', '{{ $data->nom }}', 'êtes-vous sûr de vouloir supprimer cet ville?')">
