<x-head titre="Gestion des types de categories de formations" />
<x-header />
<x-sidebar />
        ...
        <div class="page-header">
            <div class="add-item d-flex">
                <div class="page-title">
                    <h4>Gestion des types de categories de formations</h4>
                    <h6 class="text-capitalize text-muted">Paramétrage Types de categories de formations</h6>
                </div>
            </div>
            <ul class="table-top-head">
                <li>
                    <a href="{{ route('type-categories-formation.pdf') }}" target="_blank" data-bs-toggle="tooltip" data-bs-placement="top"
                        title="Imprimer on PDF"><img src="/html/assets/img/icons/pdf.svg" alt="img"></a>
                </li>
                <li>
                    <a href="{{ route('type-categories-formation.export') }}" data-bs-toggle="tooltip" data-bs-placement="top"
            ...
            <div class="page-wrapper-new p-0">
                <div class="content">
                    <div class="modal-header border-0 custom-modal-header">
                        <div class="page-title">
                            <h4>Ajouter une type de categorie formation</h4>
                        </div>
                    ...
                    <div class="modal-body custom-modal-body">
                        <form action="{{ route('type-categories-formation.add') }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="mb-3">
                                        <label class="form-label" for="nom">Catégorie de formation</label>
                                        <select name="categorie_formation_id" id="categorie_formation_id" class="form-select searchable-select"
                                            required>
                                            <option value="" disabled selected>Choisir une Catégorie de formation
                                            </option>
                                            @foreach (\App\Models\CategorieFormation::all() as $item)

                                                <option value="{{ $item->id }}">   {{$item->nom . ' - ' . $item->nom_arabe}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="mb-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var table = new DataTable('#table', {
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('type-categories-formation.data') }}",
                data: function(d) {
