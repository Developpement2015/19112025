<div class="edit-delete-action">
                ...
                <div class="page-wrapper-new p-0">
                    <div class="content">
                        ...
                        <div class="modal-body custom-modal-body">
                            <form action="{{ route('type-categories-formation.update', $data->id) }}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="mb-3">
                                            ...
                                            <select name="categorie_formation_id" id="categorie_formation_id{{ $data->id }}" class="form-select searchable-select" required>
                                                <option value="" disabled selected>Choisir un Grade</option>
                                                @foreach (\App\Models\CategorieFormation::all() as $item)
                                                <option @selected($item->id == $data->categorie_formation_id) value="{{ $item->id }}">   {{$item->nom . ' - ' . $item->nom_arabe}}</option>
                                                    </option>
                                                @endforeach
                                            </select>
    ...
    <a class="me-0 btn btn-danger  p-2 mb-0" href="javascript:void(0);"
        onclick="confirmDelete({{ $data->id }}, '{{ route('type-categories-formation.delete', $data->id) }}', '{{ $data->nom }}', 'êtes-vous sûr de vouloir supprimer cette type de catégorie de formation?')">
