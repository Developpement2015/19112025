<x-head titre="Gestion des Spécialités des cadres" />
<x-header />
<x-sidebar />
        ...
        <div class="page-header">
            <div class="add-item d-flex">
                <div class="page-title">
                    <h4>Gestion des Spécialités des cadres</h4>
                    <h6 class="text-capitalize text-muted">Paramétrage Spécialités des cadres</h6>
                </div>
            </div>
            <ul class="table-top-head">
                <li>
                    <a href="{{ route('specialite-grade.pdf') }}" target="_blank" data-bs-toggle="tooltip" data-bs-placement="top"
                        title="Imprimer on PDF"><img src="/html/assets/img/icons/pdf.svg" alt="img"></a>
                </li>
                <li>
                    <a href="{{ route('specialite-grade.export') }}" data-bs-toggle="tooltip" data-bs-placement="top"
            ...
            <div class="page-wrapper-new p-0">
                <div class="content">
                    <div class="modal-header border-0 custom-modal-header">
                        <div class="page-title">
                            <h4>Ajouter une spécialité cadre</h4>
                        </div>
                    ...
                    <div class="modal-body custom-modal-body">
                        <form action="{{ route('specialite-grade.add') }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="mb-3">
                                        <label class="form-label" for="nom">Grade</label>
                                        <select name="grade_id" id="grade_id" class="form-select searchable-select"
                                            required>
                                            <option value="" disabled selected>Choisir un grade
                                            </option>
                                            @foreach (\App\Models\GradeCadre::all() as $item)

                                               <option value="{{ $item->id }}">
   
    {{ $item->cadre?->categorie_cadre?->nom . ' - ' . $item->cadre?->categorie_cadre?->nom_arabe }} / 
    {{ $item->cadre?->nom }} / 
    {{ $item->cadre?->nom_arabe }} /
    {{ $item->nom }} /
    {{ $item->nom_arabe }}
</option>
                                            @endforeach
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var table = new DataTable('#table', {
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('specialite-grade.data') }}",
                data: function(d) {
