            ...
            <div class="page-wrapper-new p-0">
                <div class="content">
                    ...
                    <div class="modal-body custom-modal-body">
                        <form action="{{ route('arrondissements.update', $data->id) }}" method="post">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="mb-3">
                                        ...
                                        <select name="ville_id" id="ville_id"
                                            ...
                                            @foreach (\App\Models\Ville::all() as $item)
                                                <option value="{{ $item->id }}" @selected($item->id == $data->ville_id)>
...
<!-- modifier - modal-->
    <a class="me-0 btn btn-danger  p-2 mb-0" href="javascript:void(0);"
        onclick="confirmDelete({{ $data->id }}, '{{ route('arrondissements.delete', $data->id) }}', '{{ $data->nom }}', 'êtes-vous sûr de vouloir supprimer cet arrondissement/commune?')">
