<div class="edit-delete-action">
                ...
                <div class="page-wrapper-new p-0">
                    <div class="content">
                        ...
                        <div class="modal-body custom-modal-body">
                            <form action="{{ route('cadres.update', $data->id) }}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="mb-3">
                                            ...
                                            <select name="categorie_cadre_id" id="categories_cadres_id{{ $data->id }}" class="form-select searchable-select" required>
                                                <option value="" disabled selected>Choisir une catégorie cadre</option>
                                                @foreach (\App\Models\CategorieCadre::all() as $item)
                                                    <option value="{{ $item->id }}" @selected($item->id == $data->categorie_cadre_id)>
    ...
    <a class="me-0 btn btn-danger  p-2 mb-0" href="javascript:void(0);"
        onclick="confirmDelete({{ $data->id }}, '{{ route('cadres.delete', $data->id) }}', '{{ $data->nom }}', 'êtes-vous sûr de vouloir supprimer ce cadre?')">
